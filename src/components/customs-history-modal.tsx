...
export default function CustomsHistoryModal({ open, onOpenChange, article, entriesIN, factures }: CustomsHistoryModalProps) {
  const { firestore, user } = useFirebase();
  const [historyData, setHistoryData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!open || !article || !firestore || !user) return;
    ...
    const fetchData = async () => {
      ...
      // Get unique factures that have entries for this article
      ...
      try {
        await Promise.all(
          factureIds.map(async (fid: string) => {
            ...
            if (snap.exists() && snap.data().overrides && snap.data().overrides[article.id]) {
              overrideData = snap.data().overrides[article.id];
            }
            
            // Find the specific movement date
            const entry = entriesIN.find(mv => mv.factureId === fid);
  ...
  }, [open, article, firestore, user, entriesIN, factures]);

  if (!article) return null;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl p-0 overflow-hidden bg-stone-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-stone-500 mt-1">
                {article.productName} {article.color ? `- ${article.color}` : ''} {article.size ? `(${article.size})` : ''}
              </p>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-12 bg-white rounded-2xl border border-stone-100">
              <p className="text-stone-400 font-bold">Aucun dédouanement enregistré pour ce produit.</p>
            </div>
          ) : (
            <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
              <Table>
                <TableHeader className="bg-stone-50/80">
                  <TableRow>
                    <TableHead className="font-black text-[10px] uppercase text-stone-500 w-[120px]">Date</TableHead>
                    <TableHead className="font-black text-[10px] uppercase text-stone-500">Dossier / Arrivage</TableHead>
                ...
                <TableBody>
                  {historyData.map((row, i) => {
                    // Default values would normally come from the category, but we display the override if present
                    const isOverride = row.hasOverride;
                    const hsCode = isOverride && row.override.hsCode ? row.override.hsCode : article.hsCode || 'Par défaut';
                    const di = isOverride && row.override.importDuty ? `${row.override.importDuty}%` : (article.importDuty ? `${article.importDuty}%` : '—');
                    const tva = isOverride && row.override.tva ? `${row.override.tva}%` : (article.tva ? `${article.tva}%` : '20%');
