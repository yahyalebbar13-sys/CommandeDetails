      ...
      <div className="bg-white rounded-xl shadow-sm border border-stone-100 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-l-8 border-l-blue-500">
        ...
        <div className="flex gap-4">
          <div className="bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
            ...
            <div className="text-xl font-black text-blue-700">{(Number(stats.cbm) || 0).toLocaleString('en-US', { maximumFractionDigits: 3 })} m³</div>
          </div>
                  ...
                  return (
                    <TableRow key={o.id} className="hover:bg-blue-50/10 transition-colors group border-stone-50">
                      ...
                      <TableCell className="text-right font-black text-xs py-3">
                        {(Number(o.quantity) || 0).toLocaleString()} <span className="text-[9px] text-stone-400 font-bold ml-1 uppercase">{o.unitOfMeasure}</span>
                      </TableCell>
                      <TableCell className="text-right text-emerald-700 font-black text-xs py-3">{(Number(o.cubicMeasurement) || 0).toLocaleString('en-US', { maximumFractionDigits: 3 })} <span className="text-[9px] font-bold text-stone-300 ml-0.5 uppercase">m³</span></TableCell>
                      <TableCell className="text-right font-black text-amber-700 bg-amber-50/10 text-xs py-3 px-6">
                        {((Number(o.quantity) || 0) * (Number(o.purchasePricePerUnit) || 0)).toLocaleString('en-US', { maximumFractionDigits: 3 })} $
                      </TableCell>
                      <TableCell className="py-3">
